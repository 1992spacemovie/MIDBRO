...
void main(void) {
    ...
    while(1){
        ...
        napeti = (ADRESH<<8) + ADRESL;
        napeti = napeti * 5;
        napeti = napeti / 1024;
        
        uint16_t napetiSetiny = (uint16_t)(napeti*100);
        uint16_t napetiJednotky = (uint16_t)napeti;

        lcd_write_to_place(0,(char*)&napeti1);
        sprintf((char*)&napeti2, "U=%d.%02d V", napetiJednotky, napetiSetiny%100);
